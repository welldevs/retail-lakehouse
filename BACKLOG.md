...
| **Observability (OpenTelemetry, Grafana, Prometheus)** | Desirable, and explicitly **must not delay functional closure**. A dashboard existing just to generate a screenshot is the opposite of the point. | Before instrumenting: prove useful signals exist — latency, error, throughput, failure, pipeline state. Only then `application → OTel → Collector → backend`. |
