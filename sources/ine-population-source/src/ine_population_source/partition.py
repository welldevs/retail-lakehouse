...
Sem segundo eixo: a API Tempus3 do INE devolve todas as provincias e todos os anos num
unico payload por table_id, entao nao ha um eixo de fetch (como o `wh=` da Mercadona)
para promover a diretorio. Cada `table_id` vira um arquivo dentro da particao, nao um
diretorio proprio.
...
# Nome de arquivo de tabela, para reconhecer arquivos ja presentes na particao.
TABLE_FILE = re.compile(r"^table_id=([A-Za-z0-9][A-Za-z0-9_-]{0,63})\.json$")
...
def table_path(partition: str, table_id) -> str:
    """Caminho do arquivo de uma tabela, com o id validado como token seguro."""
    validate_token("table_id", str(table_id))
    return os.path.join(partition, TABLES_DIR, f"table_id={table_id}.json")
    ...
    history.append(
        {
            ...
            "table_files": totals.get("table_files"),
            "series_count": totals.get("series_count"),
        }
    )
    return history
