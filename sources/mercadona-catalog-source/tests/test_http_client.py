...
from mercadona_catalog_source import http_client
...
class UrlBuildingTest(unittest.TestCase):
    def test_parametros_sao_ordenados_e_codificados(self):
        ...
        with mock.patch("urllib.request.urlopen", fake_urlopen):
            client.get_json("/categories/", {"wh": "mad1", "lang": "es"})
        self.assertTrue(capturado["url"].endswith("/categories/?lang=es&wh=mad1"))
...
class RetryPolicyTest(unittest.TestCase):
    ...
    def test_410_tambem_falha_de_imediato(self):
        client = self._client()
        with mock.patch("urllib.request.urlopen", side_effect=http_error(410)):
            with mock.patch("time.sleep", self.dormidas.append):
                _, error = client.get_json("/x/", {})
        self.assertEqual(error, "HTTP 410")
        self.assertEqual(client.requests, 1)
    ...
    def test_corpo_json_invalido_e_retentado(self):
        ...
        quebrado.read.return_value = b"<html>bloqueado</html>"
...
class ThrottleTest(unittest.TestCase):
    def test_primeira_requisicao_nao_dorme(self):
        dormidas: list[float] = []
        client = http_client.Fetcher(1.5, 5, 0, lambda m: None)
    ...
    def test_segunda_requisicao_completa_o_intervalo(self):
        dormidas: list[float] = []
        relogio = iter([100.0, 100.0, 100.4, 100.4])
        client = http_client.Fetcher(1.5, 5, 0, lambda m: None)
        with mock.patch("urllib.request.urlopen", return_value=response({})):
            with mock.patch("time.sleep", dormidas.append):
        ...
        self.assertAlmostEqual(dormidas[0], 1.1, places=6)
    ...
    def test_marca_o_relogio_no_inicio_da_requisicao(self):
        # start-to-start: o carimbo e anterior ao urlopen, nao posterior.
        client = http_client.Fetcher(1.5, 5, 0, lambda m: None)
        marcas = []
