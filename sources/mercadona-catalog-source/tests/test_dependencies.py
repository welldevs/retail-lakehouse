...
import mercadona_catalog_source

PACKAGE_ROOT = os.path.dirname(os.path.abspath(mercadona_catalog_source.__file__))
TESTS_ROOT = os.path.dirname(os.path.abspath(__file__))
# tests/ fica diretamente sob a raiz do projeto, independente de o pacote estar
# instalado ou ser usado via PYTHONPATH.
PROJECT_ROOT = os.path.dirname(TESTS_ROOT)

# Nomes de primeiro nivel que nao vem da stdlib mas sao do proprio projeto.
LOCAL_MODULES = {"mercadona_catalog_source", "tests"}
...
class NoRuntimeDependenciesTest(unittest.TestCase):
    ...
    def test_encontrou_modulos_para_analisar(self):
        ...
        self.assertGreaterEqual(len(python_files(PACKAGE_ROOT)), 7)
        self.assertGreaterEqual(len(python_files(TESTS_ROOT)), 6)

    def test_pyproject_declara_dependencies_vazio(self):
        # Le o arquivo direto, em vez do metadata instalado: assim o teste vale tambem
        # quando o pacote nao esta instalado, e nunca vira um skip silencioso.
    ...
    def test_metadata_instalado_nao_declara_dependencia(self):
        ...
        try:
            requires = metadata.distribution("mercadona-catalog-source").requires
...
class ProjectMetadataConsistencyTest(unittest.TestCase):
    ...
    def test_versao_do_pacote_bate_com_o_pyproject(self):
        self.assertEqual(
            mercadona_catalog_source.__version__,
    ...
    def test_console_script_aponta_para_o_ponto_de_entrada_real(self):
        alvo = self._pyproject()["project"]["scripts"]["mercadona-catalog-source"]
    ...
    def test_pacote_declarado_no_pyproject_existe_em_src(self):
        ...
        self.assertTrue(
            os.path.isdir(os.path.join(PROJECT_ROOT, "src", "mercadona_catalog_source"))
        )
