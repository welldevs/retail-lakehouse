...
| `birth_year` | ano de nascimento | `ano(ingestion_date) − idade amostrada` |
...
warehouse → município ponderado pela população municipal observada (INE 29005)
          ...
          → idade pela distribuição PROVINCIAL (INE 31304) — proxy
...
| Idade | distribuição **provincial** (31304, `year=2022`, `reference_date` 2022-06-30, `fk_periodo=27`, `fk_tipo_dato=2`) | **proxy** |

Três aproximações declaradas, nenhuma escondida:
